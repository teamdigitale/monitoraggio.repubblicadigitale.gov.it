...
public class FilterUtil {
	...
	// metodo per il problema di timeout su ambiente di DEV per il match delle
	// REGEXP per far passare le api per anonimo
		...
		if (endpointQuestionarioCompilato.length > 3) {
			if (endpointQuestionarioCompilato[1].equals("servizio") &&
				...
				if ((endpointQuestionarioCompilato.length == 6 && endpointQuestionarioCompilato[5].equals("anonimo")) ||
						(endpointQuestionarioCompilato.length == 7
								&& endpointQuestionarioCompilato[6].equals("anonimo")))
					return true;
			}
		}
		return false;
	...
	// metodo per il problema di timeout su ambiente di DEV per il match delle
	// REGEXP per fare il check
	// dell'api
	// "^/servizio/cittadino/questionarioCompilato/(([A-Za-z0-9]+(\\-?)){1,})/compila$"
	...
	public boolean verificaSceltaProfilo(String codiceFiscale, String codiceRuolo, String body)
		...
		log.debug(
				"nodoIdProgramma : {} , nodoIdProgetto : {}, nodoIdEnte : {}, idProgramma : {}, idProgetto: {}, idEnte : {}",
				nodoIdProgramma, nodoIdProgetto, nodoIdEnte, idProgramma, idProgetto, idEnte);

		// esclusione api creazione
		boolean sceltaProfiloEmpty = idProgramma == null && idProgetto == null && idEnte == null;
		log.debug("Scelta profilo: {}", sceltaProfiloEmpty);
		switch (codiceRuolo) {
			case "REG":
			case "DEG":
				if (sceltaProfiloEmpty || brokenAccessControlRepository.isRefDegProgramma(codiceFiscale, codiceRuolo,
						idProgramma, idEnte) > 0)
					return true;
				break;
			case "REGP":
			case "DEGP":
				if (sceltaProfiloEmpty || brokenAccessControlRepository.isRefDegProgetto(codiceFiscale, codiceRuolo,
						idProgramma, idProgetto, idEnte) > 0)
					return true;
				break;
			case "REPP":
			case "DEPP":
				if (sceltaProfiloEmpty || brokenAccessControlRepository.isRefDegPartner(codiceFiscale, codiceRuolo,
						idProgramma, idProgetto, idEnte) > 0)
					return true;
				break;
			case "FAC":
			case "VOL":
				if (sceltaProfiloEmpty || brokenAccessControlRepository.isFacVolProgettoAndEnte(codiceFiscale,
						codiceRuolo, idProgramma, idProgetto, idEnte) > 0)
					return true;
				break;
			default:
				return true;
		}
		log.debug("Sta andando nel false");
		return false;
	}
}
