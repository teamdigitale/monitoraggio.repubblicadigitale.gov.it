...
public interface CittadinoServizioRepository extends JpaRepository<CittadinoEntity, Long> {
	@Query(value = "SELECT DISTINCT c.id as idCittadino, c.codice_fiscale as codiceFiscale, q_c.data_ora_aggiornamento as dataUltimoAggiornamento, q_c.id as idQuestionario, q_c.stato as statoQuestionario FROM servizio s INNER JOIN servizio_x_cittadino s_x_c ON s.ID = s_x_c.id_servizio INNER JOIN cittadino c ON s_x_c.id_cittadino = c.id LEFT JOIN questionario_compilato q_c ON q_c.servizio_id = s.id AND q_c.id_cittadino = c.id WHERE 1=1 AND s.id = :idServizio AND ( :criterioRicercaServizio IS NULL OR UPPER(c.codice_fiscale) = UPPER( :criterioRicercaServizio ) ) AND ( COALESCE( :statiQuestionariFiltro ) IS NULL OR q_c.STATO IN ( :statiQuestionariFiltro ) ) ORDER BY dataUltimoAggiornamento DESC", nativeQuery = true)
	...
	@Query(value = " SELECT "
			...
			+ " WHERE 1=1 "
			+ "    AND s.id = :idServizio"
			+ "    AND ( :criterioRicercaServizio IS NULL OR UPPER(c.codice_fiscale) = UPPER( :criterioRicercaServizio )) "
	...
	@Query(value = " SELECT "
			...
			+ " WHERE 1=1 "
			+ "    AND s.id = :idServizio"
			+ "    AND ( :criterioRicercaServizio IS NULL OR UPPER(c.codice_fiscale) = UPPER( :criterioRicercaServizio )) ", nativeQuery = true)
