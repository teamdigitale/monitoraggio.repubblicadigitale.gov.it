...
public interface ServizioSqlRepository extends JpaRepository<ServizioEntity, Long> {
	...
	List<ServizioEntity> findAllServiziByFacilitatoreOVolontarioAndFiltroOld(
		...
		@Query(value = " "
			...
			+ " AND (COALESCE(:statiServizioFiltro) IS NULL OR s.stato IN (:statiServizioFiltro)) "
			+ " AND (COALESCE(:idsProgettoFiltro) IS NULL OR s.ID_PROGETTO IN (:idsProgettoFiltro)) "
			+ " AND (COALESCE(:idsProgrammaFiltro) IS NULL OR s.ID_PROGETTO IN ( "
			+ "       SELECT p.ID "
			+ "       FROM progetto p "
			+ "       WHERE p.ID_PROGRAMMA IN (:idsProgrammaFiltro) "
			+ "     )) "
			+ " AND (COALESCE(:tipologieServizi) IS NULL OR EXISTS ( "
			+ "       SELECT 1 "
