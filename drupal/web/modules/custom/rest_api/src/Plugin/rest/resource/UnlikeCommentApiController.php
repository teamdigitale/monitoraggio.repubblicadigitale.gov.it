 ...
 *
 * @RestResource(
 *   id = "unlike_comment_api",
 *   label = @Translation("Api to Decrement the amount of like of a comment"),
 *   uri_paths = {
 *     "create" = "/api/comment/{id}/unlike"
 *   }
 * )
 */
class UnlikeCommentApiController extends ResourceBase
{
  /**
  ...
  public function post(Request $req, int $id)
  {
    try {
      if (empty($id)) {
        throw new Exception('UCAC02: Missing comment id', 400);
      }
      ...
      if (empty($comment)) {
        throw new Exception('UCAC03: Invalid comment id', 400);
      }
      ...
      if (empty($node)) {
        throw new Exception('UCAC05: Invalid commented node', 400);
      }

      if (!UserController::checkAuthGroups($userGroups, $route, $node->bundle())) {
        throw new Exception('UCAC06: User unauthorized', 401);
      }

      FlagController::unflag('like_comment', $comment, User::load($userId));
