...
export const generateServiceName = (serviceTypeString: string): string => {
  ...
  for (const type of serviceTypes) {
    const abbreviation = serviceAbbreviations[type] || 'Altro';
    result += (result ? '-' : 'Import_') + abbreviation;
  }

  return result + `_${date}`;
};
...
export function encryptFiscalCode(filteredRecord: CSVRecord){
  return filteredRecord.AN3 ? AES256.encrypt(filteredRecord.AN3, process?.env?.AES256_KEY) : '';
}

export function encryptDocumentAndDetermineType(filteredRecord: CSVRecord, encryptedFiscalCode: string){
  let encryptedDocNumber = '';
  let tipoDocumento = '';

  if (!encryptedFiscalCode) {
    encryptedDocNumber = filteredRecord.AN6 ? AES256.encrypt(filteredRecord.AN6, process?.env?.AES256_KEY) : '';
...
export const validateFields = (
  ...
  if (missingFields.length > 0) {
    ...
    errors.push(
      `${fieldText} "${missingFields.join(', ')}" ${verbForm} obbligator${isMultipleMissing ? 'i' : 'io'
