...
function spid_populate() {
  ...
  console.log("spid_elements",spid_elements)
  ...
  if (spid_elements.length > 0 ) {
    fetch(queryURL)
     ...
     .then(function (idps) {
      // aggiunta dello SPID Test
      idps.push({"organization_name": "SPID Test", "entity_id": "https://spid-check.facilitazione-test.repubblicadigitale.gov.it/demo", "logo_uri": ""})
...
function spid_addIdpEntry(data, element) {
  ...
  if (element.id.indexOf('get')!== -1) {
    li.innerHTML = `<a href="${servicePath}?entityID=${data['entity_id']}&return=${servicePath}"><span class="spid-sr-only">${data['organization_name']}</span><img src="${data['logo_uri']}" alt="${data['organization_name']}" /></a>`
  };
  element.prepend(li)
}
