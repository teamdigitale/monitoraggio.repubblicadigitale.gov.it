...
public interface ContestoRepository extends JpaRepository<ProgrammaEntity, Long> {
	...
	@Query(value = "SELECT p.id as idProgramma, "
			...
			+ "'Ente gestore programma' as profiloEnte "
	...
	@Query(value = "SELECT DISTINCT p.id as idProgramma, "
			...
			+ "pr.stato as statoP, "
			+ "rdgp.stato_utente as statoUtente, "
			+ "'Ente gestore di progetto' as profiloEnte "
	...
	@Query(value = "SELECT DISTINCT p.id as idProgramma, "
			...
			+ "			pr.NOME_BREVE as nomeBreveProgetto, "
			+ "			case " 
			+  "        when e.id = pr.id_ente_gestore_progetto then 'Ente gestore di progetto'\n"
			+ "         when e.id = p.id_ente_gestore_programma then 'Ente gestore di programma'\n"
			+ "         when e.id = ep.id_ente then 'Ente partner'\n"
			+ "         else ''\n"
			+ "         end as profiloEnte "
			+ "FROM progetto pr "
			...
			+ "INNER JOIN programma p "
			+ "on pr.id_programma = p.id "
			+ "left join ente_partner ep on ep.id_ente = espfl.id_ente ", nativeQuery = true)
