...
public interface UtenteRepository extends JpaRepository<UtenteEntity, Long> { 
	...
	@Query(value = " "
				 ...
				 + "     utente_and_stato.utente_id,"
				 + "	 utente_and_stato.ruolo_codice"
				 + "                             from ("
				 + "					select u.id,"
				 + "							 u.codice_fiscale,"
				 + "							 u.cognome,"
				 ...
				 + "							 u.tipo_contratto,"
				 + "							 u.consenso_dati,"
				 + "							 u.data_ora_consenso_dati,"
				 + "							 u.data_ora_aggiornamento,"
				 + "							 u.data_ora_creazione,"
				 + "							 u.integrazione,"
				 + "							 u.immagine_profilo,"
				 + "							 ruoli.utente_id,"
				 + "							 ruoli.ruolo_codice"
				 ...
				 + "												)          "
				 + "										) as ruoli "
				 + "							on u.codice_fiscale = ruoli.utente_id "
				 + " "
				 + "union "
				 + " "
				 + "						select u.id,"
				 ...
				 + "												)          "
				 + "									) as ruoli "
				 + "                            where ruoli.utente_id = u.codice_fiscale)"
				 + "			) as utente_and_stato"
				 + "            LEFT JOIN ruolo ruolo"
				 + "            ON ruolo.CODICE = utente_and_stato.RUOLO_CODICE"
				 + "            WHERE 1=1"
				 + "			AND    ("
				 + "			:criterioRicerca IS NULL"
				 + "			OR CONVERT( utente_and_stato.ID, CHAR ) = :criterioRicerca "
				 + "			OR UPPER( utente_and_stato.NOME ) LIKE UPPER( :criterioRicercaLike )"
				 + "			OR UPPER( utente_and_stato.COGNOME ) LIKE UPPER( :criterioRicercaLike ) "
				 + "			OR concat(UPPER( utente_and_stato.COGNOME ), ' ' , UPPER( utente_and_stato.NOME ))  = UPPER(:criterioRicerca)"
				 + "			OR UPPER( utente_and_stato.CODICE_FISCALE ) LIKE UPPER( :criterioRicercaLike ) "
				 + "			) "
